...
class GoGameGUI:
    ...
    def draw_lines(self, half_grid, window_end):
        ...
        for i in range(self.game_logic.board_size):
            ...
            end_x = min(window_end,
                        half_grid + (self.game_logic.board_size - 1) * self.bordConfig.GRID_CELL_SIZE())
    ...
    def draw_scores(self, grid_cell):
        ...
        """Рисование очков"""
        print(type(grid_cell), type(self.game_logic.board_size))
        black_text = font.render(f'Черные: {self.game_logic.black_score}', True, cfg.TEXT_COLOR)
        self.surface.blit(black_text, (25, max(grid_cell * self.game_logic.board_size, cfg.WINDOW_HEIGHT - 50 - 25 - 50)))
        white_text = font.render(f'Белые: {self.game_logic.white_score}', True, cfg.TEXT_COLOR)
        self.surface.blit(white_text, (25, max(grid_cell * self.game_logic.board_size + cfg.FONT_SIZE,
    ...
    def draw_buttons(self, grid_cell):
        ...
        rect = pygame.Rect(cfg.WINDOW_WIDTH // 2 - 50,
                           max(grid_cell * self.game_logic.board_size, cfg.WINDOW_HEIGHT - 50 - 25 - 50),
                           50 * 2, 50)
