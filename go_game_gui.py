...
class GoGameGUI:
    ...
    def draw(self):
        ...
        self.draw_scores(grid_cell, board_size)
        self.draw_buttons(grid_cell, board_size)
    ...
    def draw_lines(self, half_grid, window_end):
        ...
        for i in range(self.game_logic.size):
            ...
            end_x = min(window_end,
                        cfg.WINDOW_WIDTH - half_grid,
                        half_grid + (self.game_logic.size - 1) * self.bordConfig.GRID_CELL_SIZE())
            ...
            end_y = min(window_end,
                        cfg.WINDOW_HEIGHT - self.bordConfig.GRID_CELL_SIZE(),
                        half_grid + (self.game_logic.size - 1) * self.bordConfig.GRID_CELL_SIZE())

            pygame.draw.line(self.surface, cfg.LINE_COLOR, (pos, start_y), (pos, end_y), cfg.LINE_WIDTH)

    def draw_scores(self, grid_cell, board_size):
    ...
    def draw_buttons(self, grid_cell, board_size):
        font = pygame.font.Font(None, cfg.FONT_SIZE)
        """Отрисовка кнопок"""
